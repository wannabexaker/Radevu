...
test("Î¿ Î¹Î´Î¹Î¿ÎºÏ„Î®Ï„Î·Ï‚ ÎµÎ½Î·Î¼ÎµÏÏŽÎ½ÎµÎ¹ Ï€ÏÎ¿Ï†Î¯Î» ÎºÎ±Î¹ ÎšÏ…ÏÎ¹Î±ÎºÎ® Î³Î¹Î± ÎºÏÎ±Ï„Î®ÏƒÎµÎ¹Ï‚ ÎºÎ¬Ï„Ï‰ Î±Ï€ÏŒ 30 Î´ÎµÏ…Ï„ÎµÏÏŒÎ»ÎµÏ€Ï„Î±", async ({
  page
}) => {
  const { timezone } = await resetSettingsBusiness();
  const businessName = `ÎšÎ¿Ï…ÏÎµÎ¯Î¿ Î¡Ï…Î¸Î¼Î¯ÏƒÎµÏ‰Î½ ${Date.now()}`;
  const businessEmail = `settings-${Date.now()}@example.com`;
  const startedAt = Date.now();

  await page.goto("/dashboard/login");
  await page.getByLabel("Email").fill(ownerEmail);
  await page.getByLabel("ÎšÏ‰Î´Î¹ÎºÏŒÏ‚").fill(ownerPassword);
  await page.getByRole("button", { name: "Î£ÏÎ½Î´ÎµÏƒÎ·" }).click();
  await page.waitForURL("**/dashboard/today");

  await page.goto("/dashboard/settings");
  await page.getByRole("link", { name: /Î ÏÎ¿Ï†Î¯Î»/ }).click();
  await expect(
    page.getByRole("heading", { exact: true, name: "Î ÏÎ¿Ï†Î¯Î»" })
  ).toBeVisible();

  await page.getByTestId("logo-file-input").setInputFiles(tinyPngPath);
  await page.getByTestId("logo-upload-button").click();
  await expect(page.getByText("Î‘Ï€Î¿Î¸Î·ÎºÎµÏÏ„Î·ÎºÎµ")).toBeVisible();
  ...
  await expect(page.getByText("ÎŸÎ¹ Î±Î»Î»Î±Î³Î­Ï‚ Î±Ï€Î¿Î¸Î·ÎºÎµÏÏ„Î·ÎºÎ±Î½.")).toBeVisible();

  await page.goto("/qa-demo");
  await expect(page.getByRole("heading", { name: businessName })).toBeVisible();
  await expect(page.getByText(businessEmail)).toBeVisible();
  await expect(page.getByAltText(`${businessName} Î»Î¿Î³ÏŒÏ„Ï…Ï€Î¿`)).toBeVisible();

  await page.goto("/dashboard/settings");
  await page.getByRole("link", { name: /Î©ÏÎ¬ÏÎ¹Î¿ Î»ÎµÎ¹Ï„Î¿Ï…ÏÎ³Î¯Î±Ï‚/ }).click();
  ...
  const sundaySwitch = sundayCard.getByRole("switch", {
    name: "ÎšÏ…ÏÎ¹Î±ÎºÎ® Î±Î½Î¿Î¹Ï‡Ï„Î¬"
  });
  ...
  await sundayCard.getByLabel("Î‘Ï€ÏŒ").first().fill("10:00");
  await sundayCard.getByLabel("ÎˆÏ‰Ï‚").first().fill("14:00");
  await page.getByTestId("settings-hours-save").click();
  await expect(page.getByText("Î¤Î¿ Ï‰ÏÎ¬ÏÎ¹Î¿ Î±Ï€Î¿Î¸Î·ÎºÎµÏÏ„Î·ÎºÎµ.")).toBeVisible();
