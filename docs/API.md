...
Slugs `ioannis`, `giannis`, and `despoina` are pre-claimed by the founder for the live demo. Registration rejects them with `400 SLUG_RESERVED`.

| Method | Path | Auth | Body / Query | Returns |
|--------|------|------|--------------|---------|
| `POST` | `/api/v1/businesses` | None (registration) | `{ owner_email, owner_password, business_name, slug }` | `201 { business, session }` |
